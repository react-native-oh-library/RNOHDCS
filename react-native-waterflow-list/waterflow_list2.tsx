  ...
  return (
    <WaterFlow
      ...
      asyncHeightForItem={async item => {
        let height = 0
        console.log('waterflow-list2---asyncHeightForItem-------in')
    
        try {
          console.log('waterflow-list2---asyncHeightForItem-------try')
          height = await (new Promise<number>((resolve, reject) => {
          console.log('waterflow-list2---asyncHeightForItem-------await')
            Image.getSize(item.image_path, (_, imageHeight) => {
      
              console.log('waterflow-list2---asyncHeightForItem-------getSize')
              resolve(imageHeight)
            }, reject)
          }))
        } catch (err) { console.log({ err });
        console.log('waterflow-list2---asyncHeightForItem-------error')		 }
        console.log('waterflow-list2---asyncHeightForItem-------height'+height)
        return height;
      }}
	  /** 如果高度已知则传此方法 */
      heightForItem={item => {
          console.log('waterflow-list2---heightForItem-------'+item.height)
          return item.height;
      }}
      columnFlatListProps={{
...
const renderItem = item => {
  ...
  return (
    <View style={{ marginHorizontal: 5, paddingTop: 10 }}>
      <Image source={require('../../assets/expo.png')} style={{ height: item.height ,width: `100%` }}  />
