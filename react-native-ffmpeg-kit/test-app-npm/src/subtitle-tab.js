...
export default class SubtitleTab extends React.Component {
    ...
    logCallback = (log) => {
        ffprint(`${log.getSessionId()} -> ${log.getMessage()}`);
    }
    ...
    burnSubtitles = () => {
		...
		ffprint(`Testing SUBTITLE burning image1Path: ${image1Path}`);
		ffprint(`Testing SUBTITLE burning image2Path: ${image2Path}`);
		ffprint(`Testing SUBTITLE burning image3Path: ${image3Path}`);
		ffprint(`Testing SUBTITLE burning subtitlePath: ${subtitlePath}`);
        ...
        FFmpegKit.executeAsync(ffmpegCommand, async (session) => {
                ...
                if (ReturnCode.isSuccess(returnCode)) {
                    ...
                    //let burnSubtitlesCommand = `-y -i /data/storage/el2/base/haps/entry/cache/video.mp4 -vf subtitles=filename='/data/storage/el2/base/haps/entry/cache/subtitle.srt':force_style='FontName=MyFontName' -c:v mpeg4 /data/storage/el2/base/haps/entry/cache/video-with-subtitles.mp4`;
                    ...
                    FFmpegKit.executeAsync(burnSubtitlesCommand, async (secondSession) => {
						...
						ffprint(`Testing SUBTITLE burning secondSessionID: ${secondSession.getSessionId()}`);
