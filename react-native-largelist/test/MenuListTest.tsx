...
import { foods } from '../RNlargelistExample/LargeListExamples/DataSource';
import { LargeList } from 'react-native-largelist';

const leftData = [{ items: foods }];

export default class MenuListTest extends Component {
  selectedIndex: number = 0;
  _listRef: LargeList | undefined | null;
  indexes: LargeList | undefined | null;
  _buttonRefs: [] = [];
  ...
  renderIndexes = ({ section: section, row: row }) => {
    ...
    return (
      <TouchableOpacity
        ...
        onPress={() => {
          this._listRef!.scrollToIndexPath({ section: row, row: -1 }, true).then();
        }}>
      ...
      <View style={{ flex: 1 }}>
        <View style={{ flex: 1, flexDirection: 'row' }}>
          ...
          <View style={{ flex: 1, marginLeft: 5, marginTop: 10 }}>
            ...
            <Text style={{ fontSize: 14, color: '#666' }}>{food.subtitle}</Text>
            <View style={{ flexDirection: 'row' }}>
              <Text style={{ fontSize: 14, color: '#666' }}>{food.sales}</Text>
            ...
            <View
              ...
              <Text style={{ fontSize: 16, color: 'red' }}>{food.prise}</Text>
              <TouchableOpacity
                style={{
