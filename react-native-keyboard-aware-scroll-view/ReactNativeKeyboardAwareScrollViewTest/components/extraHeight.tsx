...
export const extraHeight: React.FC = (): JSX.Element => {
    ...
    return <Tester>
        <TestSuite name='react-native-autoheight-webview'>
            <TestCase
                key={'getInitStatus_1'}
                itShould={`对TextInputs聚焦时添加额外的偏移量`}
                tags={['C_API']}
                initialState={false}
                ...
                arrange={({ setState }) => {
                    ...
                    return (
                        ...
                        >
                            <TextInput keyboardType='numeric' style={styles.input} placeholder='请输入extraHeight' onEndEditing={(val) => { setExtraHeight(Number(val.nativeEvent.text)); setState(true) }} />
                            {
                                Labels.map(item => {
