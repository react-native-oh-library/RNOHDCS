  ...
  return (
    <Tester>
      <ScrollView style={styles.content}>
        ...
        {/* <TestSuite name="验证shareSingle接口-三方分享文档">
          <TestCase.Manual
            itShould="true"
            initialState={false}
            tags={['dev']}
            arrange={({ setState }) => {
              return (
                <View style={styles.section}>
                  <Button
                    key='openImageAction62'
                    title='三方分享文档'
                    onPress={async () => {
                      const res = await shareAction({social:share,"url":txtUrl,title,subject});
                      setState(res)
                    }}
              ...
              return (
                <View style={styles.section}>
                  <Button
                    ...
                    title='系统分享文档'
                    onPress={async () => {
                      const res = await openAction({title,
                        subject,
                        url:docUrl,
                        saveToFiles: false});
                      setState(res);
