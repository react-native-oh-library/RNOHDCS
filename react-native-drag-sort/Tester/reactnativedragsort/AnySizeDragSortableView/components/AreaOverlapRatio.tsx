...
export const API_areaOverlapRatio = () => {
  ...
  return (
    <Tester>
      <TestSuite name="areaOverlapRatio">
        <TestCase
          key={'areaOverlapRatio'}
          itShould={`属性areaOverlapRatio 当前为${areaOverlapRatio}`}
          tags={['C_API']}
          initialState={false}
          arrange={({setState}) => {
            return (
              <View style={{height: 510}}>
                ...
                />
                <Button
                  title="修改areaOverlapRatio"
                  onPress={() => {
                    setAreaOverlapRatio(0.65);
                    setState(true);
                  }}
                />
              </View>
