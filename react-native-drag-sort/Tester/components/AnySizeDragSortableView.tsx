...
const getW = (index: number, isWidth: boolean) =>
  isWidth ? (width - 170) / 2 : 80;
...
const renderHeaderViewComp = (
  <View style={styles.aheader}>
    <Text style={styles.aheader_title}>我是头部组件</Text>
  </View>
);
const renderBottomViewComp = (
...
export class T_AnySizeDragSortableView extends Component {
  constructor(props: {}) {
    ...
    this.state = {
      ...
      scrollIndicatorInsets: {top: 50, left: 50, bottom: 50, right: 50},
      childMarginTop: 0,
      childMarginBottom: 0,
      childMarginLeft: 0,
      childMarginRight: 0,
  ...
  _renderItem = (
    ...
    return (
      <TouchableOpacity
        ...
        onPressOut={() => {
          if (viewRef) {
            this[viewRef]?.current.onPressOut();
          }
        }}>
        <View
          style={[
          ...
          <View
            style={[
              styles.item,
              {
                ...
                backgroundColor: isMoved ? 'green' : '#f39c12',
              },
            ]}>
            {isMoved ? (
      ...
      <Tester>
        <TestSuite name="react-native-drag-sort-AnySizeDragSortableView">
          <TestCase
            ...
            itShould={`change areaOverlapRatio 当前为${areaOverlapRatio}`}
            tags={['C_API']}
            initialState={false}
            arrange={({setState}) => {
              return (
          ...
          <TestCase
            ...
            itShould={`change scrollIndicatorInsets 当前为${scrollIndicatorInsets}`}
            tags={['C_API']}
            initialState={false}
            arrange={({setState}) => {
              return (
