...
import { StyleSheet, View, Switch, Text, ScrollView, Alert } from "react-native";
import { CreditCardInput, LiteCreditCardInput, CardView, CreditCardFormData } from "react-native-credit-card-input";
...
export const ReactNativeCreditCardInputTest = () => {
  ...
  const handleFocus = () => {
    if (!hasShownAlert) {
      Alert.alert(`Input focused`);
      setHasShownAlert(true); // 设置状态，表示已经显示过弹窗
    }
  };
  ...
  return (
    <ScrollView>
      <Tester style={{ flex: 1 }}>
          ...
          <TestCase
            ...
            <View>
              <CreditCardInput cardBrandIcons={
                {
                  "mastercard": require("./icons/stp_card_cvc.png"),
                }
              } />
            </View>
          </TestCase>
        ...
        <TestSuite name="CreditCardInput22">
          <TestCase
            tags={['C_API']}
            itShould="CreditCardInput中 onFocus">
            <View>
              <CreditCardInput
                onFocus={handleFocus}
              />
        ...
        <TestSuite name="CreditCardInput23">
          <TestCase
            tags={['C_API']}
            itShould="CreditCardInput中 cardImageBack">
            <View>
              <CreditCardInput
                cardImageBack={require('./icons/stp_card_cvc.png')}
              />
            </View>
          </TestCase>
        </TestSuite>
...
function CheckHandleFormSubmit() {
  ...
  const handleFormSubmit = (postalCode) => {
    // 假设这里是表单提交的处理函数
    const postalCode1 = '123456'; // 假设从表单获取邮政编码
