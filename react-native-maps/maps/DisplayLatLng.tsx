...
class DisplayLatLng extends React.Component<any, any> {
  ...
  onRegionChange(region: any) {
    let _region = region.nativeEvent.region;
    if(_region){
      _region.latitudeDelta = LATITUDE_DELTA;
      _region.longitudeDelta = LONGITUDE_DELTA;
    }
  ...
  animateRandomCoordinate() {
    this.map.animateCamera({center: this.randomCoordinate(), zoom: 12});
  }

  animateToRandomBearing() {
    this.map.animateCamera({center: this.randomCoordinate(), zoom: 12, heading: this.getRandomFloat(-360, 360)});
  }

  animateToRandomViewingAngle() {
    Alert.alert('华为地图不支持 pitch 参数')
    this.map.animateCamera({center: this.randomCoordinate(), zoom: 12, pitch: this.getRandomFloat(0, 90)});
  }
